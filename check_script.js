...
function renderToday() {
  ...
  if (prepDow === 2 || prepDow === 6) {
    ...
    const doneCount = prep.tasks.filter(task => prepState[task.id]).length;
    html += `<div class="card" style="background:rgba(42,106,181,0.07);border-color:rgba(42,106,181,0.28)">
      ...
      <div style="font-size:11px;color:var(--text3);line-height:1.45;margin-bottom:10px">${doneCount}/${prep.tasks.length} prep tasks done. Keep the detailed list in Plan so Today stays clean.</div>
...
function getPrepChecklist(slot, wn=curWk()) {
  ...
  const countByMeal = id => dayPlans.reduce((sum, day) => sum + day.meals.filter(m => m.id === id).length, 0);
  ...
  const focusIngredients = Object.entries(ingredientCounts)
    ...
    .slice(0,6)
    .map(([name]) => name);
  const lunchDinnerCount = countByMeal('m3') + countByMeal('m5');
  const snackCount = countByMeal('m2') + countByMeal('m4');
  return {
    ...cfg,
    dayPlans,
    focusIngredients,
    tasks:[
      {id:'mains', label:`Cook ${lunchDinnerCount} main meal portions`, note:'Batch proteins + carbs into containers so lunch and dinner are already decided.'},
      {id:'breakfast', label:`Prep ${countByMeal('m1')} breakfast starts`, note:'Set up yoghurt pots, toast add-ons, or the ingredients you need ready to grab.'},
      {id:'snacks', label:`Build ${snackCount} snack / shake packs`, note:'Pre-portion oats, whey, fruit, rice cakes, and training snacks.'},
      {id:'veg', label:`Prep veg and sides for ${lunchDinnerCount} meals`, note:'Wash, chop, and portion the veg that slows you down when busy.'},
      {id:'label', label:`Label containers for ${cfg.subtitle}`, note:'A simple Mon/Tue/Wed or Thu/Fri/Sat/Sun label makes adherence much easier.'}
    ]
  };
}
function renderPrepSlot(slot, mountId) {
  ...
  const doneCount = prep.tasks.filter(task => state[task.id]).length;
  const pct = Math.round((doneCount / prep.tasks.length) * 100);
  mount.innerHTML = `
    <div class="prep-progress">
      <div>
        <div style="font-size:12px;color:var(--text2)">${prep.subtitle}</div>
        <strong style="color:${pct>=80?'var(--green)':pct>=40?'var(--gold)':prep.accent}">${doneCount}/${prep.tasks.length}</strong>
      </div>
    ...
    ${prep.tasks.map(task => `
      <div class="prep-item">
        <div class="prep-main">
          <div class="prep-name">${task.label}</div>
          <div class="prep-note">${task.note}</div>
        </div>
        <div class="prep-toggle ${state[task.id]?'on':''}" onclick="togglePrepItem('${slot}','${task.id}')"></div>
      </div>`).join('')}
  ...
  nextMount.innerHTML = `
    <div style="display:flex;align-items:center;justify-content:space-between;gap:10px">
      <div>
        ...
        <div style="font-size:12px;color:var(--text3)">${prep.subtitle}</div>
      </div>
